...
class OGM2D_V3:
    ...
    def __init__(self, config: DictConfig, world_size: Tuple[float],
        ...
        assert isinstance(config.vsa_dimensions, int)
        assert config.vsa_dimensions > 0
        assert any(isinstance(config.decision_thresholds, t) for t in [list, tuple, ListConfig])
        assert len(config.decision_thresholds) == 2
        assert all(any(isinstance(config.decision_thresholds[i], t) for t in [float, int]) for i in range(2))
        assert config.decision_thresholds[0] < config.decision_thresholds[1]
        assert config.decision_thresholds[0] > -1
        assert config.decision_thresholds[1] < 1
        assert isinstance(config.verbose, bool)
        assert isinstance(config.plotting.plot_xy_voxels, bool)
        ...
        # --------------------------------------
        # Configuration Defined Class Variables
        # --------------------------------------
        # self.label_vectors: list = [
        #     spf.make_good_unitary( # empty
        #         num_dims=self.vsa_dimensions,
        #         device=self.device,
        #         length_scale=self.length_scale,
        #     ),
        #     spf.make_good_unitary(  # occupied
        #         num_dims=self.vsa_dimensions,
        #         device=self.device,
        #         length_scale=self.length_scale,
        #     ),
        ...
        self.obs_log: dict = {
            "runtime": {
                "class_separation": np.zeros(shape=(self.obs_hist_len,)),
                "counting": np.zeros(shape=(self.obs_hist_len,)),
                "bounds_filtering": np.zeros(shape=(self.obs_hist_len,)),
                "encoding": np.zeros(shape=(self.obs_hist_len,)),
                "label_filtering": np.zeros(shape=(self.obs_hist_len,)),
                "info_filtering": np.zeros(shape=(self.obs_hist_len,)),
                "occupancy_probability_calculation": np.zeros(shape=(self.obs_hist_len,)),
                "empty_probability_calculation": np.zeros(shape=(self.obs_hist_len,)),
                "tau_calculation": np.zeros(shape=(self.obs_hist_len,)),
                "pc_normalization": np.zeros(shape=(self.obs_hist_len,)),
                "pc_rounding": np.zeros(shape=(self.obs_hist_len,)),
                "quadrant_center_calculation": np.zeros(shape=(self.obs_hist_len,)),
                "unique_points": np.zeros(shape=(self.obs_hist_len,)),
                "classification": np.zeros(shape=(self.obs_hist_len,)),
            },
            "point_clouds": {
                "originals": [],
                "originals_normalized": [],
                "originals_normalized_subsampled": [],
                "ons_occupied": [],
                "ons_empty": [],
                "unique_ops": [],
                "unique_ops_counts": [],
                ...
                "unique_eps_counts": [],        
                "unique_points": [],
                "unique_points_counts": [],
                "conclusive_points": [],
                "conclusive_points_labels": [],
                "info_rich_points": [],
                "info_rich_points_labels": [],
            },
            "obs_proc": {
                "quadrant_centers_matrix": [],
                "quadrant_memory_vectors": [],
    ...
    def process_observation(self, point_cloud: Union[np.ndarray, torch.tensor],
        ...
        # ------------------------------------------------------------
        # rounding input points to arbitrary precision below the axis
        # resolution seen within literature (~0.1 meters)
        # ------------------------------------------------------------
        # round_time_start: float = time.time()

        # if False:
        #     point_cloud = torch.round(point_cloud, decimals=1)

        # round_time_end: float = time.time()
        # round_time: float = round_time_end - round_time_start
        # self.obs_log["runtime"]["pc_rounding"][self.obs_count] += round_time

        # # log the normalized point cloud rounded and subsampled
        # ogpcns: torch.tensor = point_cloud.clone()
        # ogpcns: np.ndarray = ogpcns.detach().cpu().numpy()
        # self.obs_log["point_clouds"]["originals_normalized_subsampled"].append(ogpcns)

        # ---------------------------------------------
        # separate the occupied and empty points
        # ---------------------------------------------
        # sep_time_start: float = time.time()

        # ops: torch.tensor = point_cloud[labels == 1]
        # eps: torch.tensor = point_cloud[labels == 0]

        # sep_time_end: float = time.time()
        # sep_time: float = sep_time_end - sep_time_start
        # self.obs_log["runtime"]["class_separation"][self.obs_count] += sep_time

        # # log the occupied and empty point clouds
        # ops_copy: torch.tensor = ops.clone()
        # ops_copy: np.ndarray = ops_copy.detach().cpu().numpy()
        # self.obs_log["point_clouds"]["ons_occupied"].append(ops_copy)
        # eps_copy: torch.tensor = eps.clone()
        # eps_copy: np.ndarray = eps_copy.detach().cpu().numpy()
        # self.obs_log["point_clouds"]["ons_empty"].append(eps_copy)

        # ----------------------------------------------------
        # Filter out the points that are outside the world
        # ----------------------------------------------------
        # filter_time_start: float = time.time()

        # ops = ops[torch.where(ops[:, 0] >= 0)]
        # ops = ops[torch.where(ops[:, 0] <= self.world_bounds_norm[0])]
        # ops = ops[torch.where(ops[:, 1] >= 0)]
        # ops = ops[torch.where(ops[:, 1] <= self.world_bounds_norm[1])]

        # eps = eps[torch.where(eps[:, 0] >= 0)]
        # eps = eps[torch.where(eps[:, 0] <= self.world_bounds_norm[0])]
        # eps = eps[torch.where(eps[:, 1] >= 0)]
        # eps = eps[torch.where(eps[:, 1] <= self.world_bounds_norm[1])]

        # filter_time_end: float = time.time()
        # filter_time: float = filter_time_end - filter_time_start
        # self.obs_log["runtime"]["bounds_filtering"][self.obs_count] += filter_time
    
        # ----------------------------------------------------
        # Extract the unique occupied, empty, and both points
        # ----------------------------------------------------
        # unique_points_time_start: float = time.time()

        # unique_ops, uops_counts = torch.unique(ops, dim=0, return_counts=True)
        # unique_eps, ueps_counts = torch.unique(eps, dim=0, return_counts=True)

        # unique_ps, ups_counts = torch.unique(point_cloud, dim=0, return_counts=True)

        # unique_points_time_end: float = time.time()
        # unique_points_time: float = unique_points_time_end - unique_points_time_start
        # self.obs_log["runtime"]["unique_points"][self.obs_count] += unique_points_time

        # # log the unique points and their counts
        # unique_ops_copy: torch.tensor = unique_ops.clone()
        # unique_ops_copy: np.ndarray = unique_ops_copy.detach().cpu().numpy()
        # self.obs_log["point_clouds"]["unique_ops"].append(unique_ops_copy)
        # uops_counts_copy: torch.tensor = uops_counts.clone()
        # uops_counts_copy: np.ndarray = uops_counts_copy.detach().cpu().numpy()
        # self.obs_log["point_clouds"]["unique_ops_counts"].append(uops_counts_copy)
        # unique_eps_copy: torch.tensor = unique_eps.clone()
        # unique_eps_copy: np.ndarray = unique_eps_copy.detach().cpu().numpy()
        # self.obs_log["point_clouds"]["unique_eps"].append(unique_eps_copy)
        # ueps_counts_copy: torch.tensor = ueps_counts.clone()
        # ueps_counts_copy: np.ndarray = ueps_counts_copy.detach().cpu().numpy()
        # self.obs_log["point_clouds"]["unique_eps_counts"].append(ueps_counts_copy)
        # unique_ps_copy: torch.tensor = unique_ps.clone()
        # unique_ps_copy: np.ndarray = unique_ps_copy.detach().cpu().numpy()
        # self.obs_log["point_clouds"]["unique_points"].append(unique_ps_copy)

        # ---------------------------------------------------------
        # Account for the counts of individual empty and occupied
        # points in the unique points tensor
        # ---------------------------------------------------------    
        # counting_time_start: float = time.time()

        # if False:
        #     ups_counts = torch.zeros(unique_ps.shape[0], device=self.device)
            
        #     for i, p in enumerate(unique_ops):
        #         point_match_filter = torch.sum(unique_ps - p, dim=1)
        #         points_match = torch.where(point_match_filter == 0)
        #         ups_counts[points_match] += uops_counts[i]
            
        #     for i, p in enumerate(unique_eps):
        #         point_match_filter = torch.sum(unique_ps - p, dim=1)
        #         points_match = torch.where(point_match_filter == 0)
        #         ups_counts[points_match] -= ueps_counts[i]

        # counting_time_end: float = time.time()
        # counting_time: float = counting_time_end - counting_time_start
        # self.obs_log["runtime"]["counting"][self.obs_count] += counting_time

        # # log the counts of the unique points
        # ups_counts_copy: torch.tensor = ups_counts.clone()
        # ups_counts_copy: np.ndarray = ups_counts_copy.detach().cpu().numpy()
        # self.obs_log["point_clouds"]["unique_points_counts"].append(ups_counts_copy)

        # ---------------------------------------------------------
        # Filter out each point that has inconclusive information
        # i.e. points that have equal occupied and empty labels
        # ---------------------------------------------------------
        # filter_time_start: float = time.time()

        # if False:
        #     ups = unique_ps[ups_counts != 0]
        #     ups_labels = ups_counts[ups_counts != 0]
        #     ups_counts = ups_counts[ups_counts != 0]

        #     ups_labels[ups_counts >= 1] = 1
        #     ups_labels[ups_counts <= -1] = 0
    
        # filter_time_end: float = time.time()
        # filter_time: float = filter_time_end - filter_time_start
        # self.obs_log["runtime"]["label_filtering"][self.obs_count] += filter_time

        # assert len(ups.shape) == 2
        # assert ups.shape[1] == self.environment_dimensionality
        # assert len(ups_labels.shape) == 1
        # assert ups.shape[0] == ups_labels.shape[0]

        # # log the conclusive points and their labels
        # ups_copy: torch.tensor = ups.clone()
        # ups_copy: np.ndarray = ups_copy.detach().cpu().numpy()
        # self.obs_log["point_clouds"]["conclusive_points"].append(ups_copy)
        # ups_labels_copy: torch.tensor = ups_labels.clone()
        # ups_labels_copy: np.ndarray = ups_labels_copy.detach().cpu().numpy()
        # self.obs_log["point_clouds"]["conclusive_points_labels"].append(ups_labels_copy)
    
        ups = point_cloud
        ups_labels = labels
        
        # --------------------------------------------------------------
        # Filter out the points that don't add any relevant information
        # --------------------------------------------------------------
        filter_time_start: float = time.time()

        # unique_scaled = ups / self.axis_resolution
        # unique_scaled = torch.round(unique_scaled)
        # unique_scaled -= 1
        # unique_scaled = unique_scaled.long()

        # current_labels = self.xy_axis_class_matrix[unique_scaled[:, 0], unique_scaled[:, 1]]
        # unique_scaled = unique_scaled[torch.where(current_labels != ups_labels)]

        # ups = ups[torch.where(current_labels != ups_labels)]
        # ups_labels = ups_labels[torch.where(current_labels != ups_labels)]

        filter_time_end: float = time.time()
        filter_time: float = filter_time_end - filter_time_start
        self.obs_log["runtime"]["info_filtering"][self.obs_count] += filter_time

        # log the information rich points and their labels
        ups_copy: torch.tensor = ups.clone()
        ups_copy: np.ndarray = ups_copy.detach().cpu().numpy()
        self.obs_log["point_clouds"]["info_rich_points"].append(ups_copy)
        ups_labels_copy: torch.tensor = ups_labels.clone()
        ups_labels_copy: np.ndarray = ups_labels_copy.detach().cpu().numpy()
        self.obs_log["point_clouds"]["info_rich_points_labels"].append(ups_labels_copy)
        ...
        # ----------------------------------------------------------------
        # Method 2 - Bind the Occupied and Empty Points to the Label once
        # ----------------------------------------------------------------
        occupied_points = ups[ups_labels == 1]
        occupied_points_closest_quads = closest_quads[ups_labels == 1]
        ...
        for j, y_lower in enumerate(self.quadrant_indices_y[:-1]):
            for i, x_lower in enumerate(self.quadrant_indices_x[:-1]):
                ...
                # quadrant_heatmap = torch.square(quadrant_heatmap)
                quadrant_heatmap /= torch.max(quadrant_heatmap)
        ...
        # ----------------------------------------------
        # Calculate the tau for the global map
        # ----------------------------------------------
        tau_time_start: float = time.time()

        self.xy_axis_global_heatmap = self.xy_axis_occupied_heatmap \
            - self.xy_axis_empty_heatmap

        tau_time_end: float = time.time()
        tau_time: float = tau_time_end - tau_time_start
        self.obs_log["runtime"]["tau_calculation"][self.obs_count] += tau_time   

        # log the global map
        ghm_copy: torch.tensor = self.xy_axis_global_heatmap.clone()
        ghm_copy: np.ndarray = ghm_copy.detach().cpu().numpy()
        self.obs_log["obs_proc"]["global_maps"].append(ghm_copy) 
    
        # ----------------------------------------------
        # Update the class matrix
        # ----------------------------------------------
        classification_time_start: float = time.time()

        self.xy_axis_class_matrix[torch.where(self.xy_axis_global_heatmap > self.decision_threshold[1])] = 1
        self.xy_axis_class_matrix[torch.where(self.xy_axis_global_heatmap < self.decision_threshold[0])] = 0
        # self.xy_axis_class_matrix[torch.where((self.xy_axis_global_heatmap <= self.decision_threshold[1]) & (self.xy_axis_global_heatmap >= self.decision_threshold[0]))] = 0

        classification_time_end: float = time.time()
        classification_time: float = classification_time_end - classification_time_start
        self.obs_log["runtime"]["classification"][self.obs_count] += classification_time

        # log the class matrix
        cm_copy: torch.tensor = self.xy_axis_class_matrix.clone()
        cm_copy: np.ndarray = cm_copy.detach().cpu().numpy()
        self.obs_log["obs_proc"]["class_matrices"].append(cm_copy)

        if self.obs_count % 50 == 0:
            pass
        # if self.obs_count > 360:
        self._plot_observation()
    ...
    def _plot_point_counts(self, base_path: str, point_counts_dir: str = "point_counts",
        ...
        plot_titles: list[str] = [
            "Original",
            "Original Normalized",
            "Original Normalized Subsampled",
            "Occupied",
            "Empty",
            "Unique Occupied",
            "Unique Empty",
    ...
    def _plot_point_clouds(self, base_path: str, point_clouds_dir: str = "point_clouds",
        ...
        # Plot the unique points with counts
        key_list: list[str] = [
            "unique_ops",
            "unique_eps",
            "unique_points",
        ...
        title_list: list[str] = [
            "Unique Occupied Points",
            "Unique Empty Points",
            "Unique Points",
        ]

        # verify that the unique points have counts
        for idx, key in enumerate(key_list):
            assert f"{key}_counts" in self.obs_log["point_clouds"].keys()

            # skip over plotting whenever the logs do not exist
            if idx >= len(self.obs_log["point_clouds"][key]):
                continue
            ...
            self._plot_point_cloud(
                ...
                colors=self.obs_log["point_clouds"][f"{key}_counts"][self.obs_count],
            )
    
            pc_np_path: str = os.path.join(
                numpy_path,
                f"{key}_pc.npy"
            )
            with open(pc_np_path, "wb") as f:
                np.save(f, self.obs_log["point_clouds"][key][self.obs_count])

            counts_np_path: str = os.path.join(
                numpy_path,
                f"{key}_counts.npy"
            )
            with open(counts_np_path, "wb") as f:
                np.save(f, self.obs_log["point_clouds"][f"{key}_counts"][self.obs_count])
            
        # Plot the labelled conclusive and information rich points
        key_list: list[str] = [
            "conclusive_points",
            "info_rich_points",
        ]

        # verify that the conclusive and information rich points have labels
        for key in key_list:
            assert f"{key}_labels" in self.obs_log["point_clouds"].keys()
        ...
        for idx, key in enumerate(key_list):
            ...
            self._plot_point_cloud(
                ...
                save_path=plot_path,
                title=title_list[idx],
                colors=self.obs_log["point_clouds"][f"{key}_labels"][self.obs_count],
            )
            ...
            labels_np_path: str = os.path.join(
                numpy_path,
                f"{key}_labels.npy"
            )
            with open(labels_np_path, "wb") as f:
                np.save(f, self.obs_log["point_clouds"][f"{key}_labels"][self.obs_count])
...
class LidarOccupancyGridMapper2D_SpatialQuadrants:
    """
    This class represents a 2D occupancy grid mapper that uses spatial
    quadrants to encode the environment.
    """
    def __init__(self, config: DictConfig, world_size: List[float]) -> None:
        """
        Initializes a Mapping object with the given configuration and world size.

        Args:
            config (DictConfig): A configuration object containing the
                following fields:
                - axis_resolution (float): The resolution of the x and y axes
                    in meters.
                - device (str): The device to use for computation (e.g. "cpu"
                    or "cuda").
                - length_scale (float): The length scale to use for generating
                    unitary vectors.
                - quadrant_sizes (List[int]): A list of integers representing
                    the number of quadrants at each level of the hierarchy.
                - vsa_dimensions (int): The number of dimensions to use for
                    the vector symbolic architecture.

            world_size (List[float]): A list of four floats representing the
                bounds of the world in meters, in the following order:
                [xmin, xmax, ymin, ymax].
        """
        ...
        self.length_scale: float = config.length_scale
        self.quadrant_sizes: List[int] = config.quadrant_sizes
        self.vsa_dimensions: int = config.vsa_dimensions

        self.pdist = torch.nn.PairwiseDistance()
        self.empty_memory = torch.zeros(self.vsa_dimensions, device=self.device)
        self.occupied_memory = torch.zeros(self.vsa_dimensions, device=self.device)
        self.world_bound_norm: Tuple[float] = (
            self.world_bounds[1] - self.world_bounds[0],
            self.world_bounds[3] - self.world_bounds[2]
        )

        self.label_vectors: dict = {
            "occupied": spf.make_good_unitary(
                num_dims=self.vsa_dimensions,
                device=self.device,
                length_scale=self.length_scale
            ),
            "empty": spf.make_good_unitary(
                num_dims=self.vsa_dimensions,
                device=self.device,
                length_scale=self.length_scale
            )
        ...
        self.ssp_gen = SSPGenerator(
            dimensionality=self.vsa_dimensions,
            device=self.device,
            length_scale=self.length_scale
        )

        # quadrant_level_basis_vectors: torch.tensor = []
        # for _ in range(len(self.quadrant_sizes)):
        #     vx = spf.make_good_unitary(
        #         self.vsa_dimensions,
        #         self.device,
        #         length_scale=self.length_scale
        #     )
        #     vy = spf.make_good_unitary(
        #         self.vsa_dimensions,
        #         self.device,
        #         length_scale=self.length_scale
        #     )
        #     quadrant_level_basis_vectors.append((vx, vy))

        self.quadrant_centers: list[torch.tensor] = []
        self.quadrant_vectors: list[torch.tensor] = []
        quadrant_sizes: list[float] = []
        for level, size in enumerate(self.quadrant_sizes):
            
            # Calculate the quadrant size across x and y axes in meters
            # while also taking into account the level of the quadrant
            if level == 0:
                size_x: float = self.world_bound_norm[0] / size / self.axis_resolution
                size_y: float = self.world_bound_norm[1] / size / self.axis_resolution
            else:
                size_x: float = quadrant_sizes[level - 1][0] / size / self.axis_resolution
                size_y: float = quadrant_sizes[level - 1][1] / size / self.axis_resolution

            quadrant_sizes.append((size_x, size_y))

            # calculate the quadrant centers
            qcs_x = torch.linspace(0, self.world_bound_norm[0], size + 2)[1:-1]
            qcs_y = torch.linspace(0, self.world_bound_norm[1], size + 2)[1:-1]
            qcs = torch.stack(torch.meshgrid(qcs_x, qcs_y), dim=2)
            qcs = qcs.to(self.device)
            self.quadrant_centers.append(qcs)

            qvm = torch.zeros((size_x, size_y, self.vsa_dimensions), device=self.device)

            print(f"Quadrant Level [{level}] - Size: {qvm.shape}")

            # calculate the quadrant vectors for the current level by
            # binding the quadrant level basis vectors to the quadrant indices
            # qlbv_x: torch.tensor = quadrant_level_basis_vectors[level][0]
            # qlbv_y: torch.tensor = quadrant_level_basis_vectors[level][1]
            for i in range(qvm.shape[0]):
                for j in range(qvm.shape[1]):
                    # vs = [
                    #     spf.power(qlbv_x, i, self.length_scale),
                    #     spf.power(qlbv_y, i, self.length_scale)
                    # ]
                    # qvm[i, j, :] = spf.bind(vs, self.device)

                    qvm[i, j, :] = spf.make_good_unitary(
                        self.vsa_dimensions,
                        self.device,
                        length_scale=self.length_scale
                    )

            self.quadrant_vectors.append(qvm)

            print(f"Quadrant Level [{level}] - Size: {size_x}, {size_y}")
            print(f"Quadrant Level [{level}] - Centers Shape: {qcs.shape}")
            print(f"Quadrant Level [{level}] - Vectors Shape: {qvm.shape}")

        # build the global xy axis matrix
        x_axis = torch.linspace(
            start=0,
            end=self.world_bound_norm[0],
            steps=int(self.world_bound_norm[0] / self.axis_resolution)
        )
        y_axis = torch.linspace(
            start=0,
            end=self.world_bound_norm[1],
            steps=int(self.world_bound_norm[1] / self.axis_resolution)
        )
        
        self.axis_basis_vectors: torch.tensor = self.ssp_gen.generate(2)
        self.xy_axis_matrix = torch.zeros(
            (x_axis.shape[0], y_axis.shape[0], self.vsa_dimensions),
            device=self.device
        )
        ...
        for i, x in enumerate(x_axis):
            for j, y in enumerate(y_axis):
                ...
                # calculate the corresponding quadrant basis vectors
                vs: list[torch.tensor] = [
                    spf.power(self.axis_basis_vectors[0], x, self.length_scale),
                    spf.power(self.axis_basis_vectors[1], y, self.length_scale)
                ]

                for level in range(len(self.quadrant_centers)):
                    distances = self.pdist(torch.tensor([x, y], device=self.device), self.quadrant_centers[level])
                    closed_quad = torch.argwhere(distances == torch.min(distances))[0]
                    vs.append(self.quadrant_vectors[level][closed_quad[0], closed_quad[1], :])

                self.xy_axis_matrix[i, j, :] = spf.bind(vs, self.device)

        print(f"Axis Matrix Shape: {self.xy_axis_matrix.shape}")        
    
    def process_observation(self, point_cloud: Union[np.ndarray, torch.tensor],
                            labels: Union[np.ndarray, torch.tensor]) -> None:
        """
        Processes an observation represented as a point cloud and its corresponding labels.

        Args:
            point_cloud (Union[np.ndarray, torch.tensor]): The point cloud representing the observation.
            labels (Union[np.ndarray, torch.tensor]): The labels corresponding to each point in the point cloud.
        ...
        point_cloud = torch.from_numpy(point_cloud) if isinstance(point_cloud, np.ndarray) else point_cloud
        ...
        for i in range(point_cloud.shape[0]):
            ...
            for level in range(len(self.quadrant_centers)):
                distances = self.pdist(point_cloud[i, :], self.quadrant_centers[level])
                closed_quad = torch.argwhere(distances == torch.min(distances))[0]
                vs.append(self.quadrant_vectors[level][closed_quad[0], closed_quad[1], :])

            vs.append(spf.power(self.axis_basis_vectors[0], point_cloud[i, 0], self.length_scale))
            vs.append(spf.power(self.axis_basis_vectors[1], point_cloud[i, 1], self.length_scale))
            
            if labels[i] == 1:
                # vs.append(self.label_vectors["occupied"])
                self.occupied_memory += spf.bind(vs, self.device)
            else:
                # vs.append(self.label_vectors["empty"])
                self.empty_memory += spf.bind(vs, self.device)

            # self.memory += spf.bind(vs, self.device)

    def decode_occupied(self, get_heat_map: bool = False) -> torch.tensor:
        """
        Decodes the empty space in the map.

        Args:
            get_heat_map (bool): If True, returns the decoded tensor multiplied by the axis matrix.

        Returns:
            torch.tensor: The decoded tensor.
        """
        vs = [spf.invert(self.qu), self.memory]
        decoded = spf.bind(vs, self.device)

        if get_heat_map:
            decoded = torch.tensordot(self.occupied_memory, self.xy_axis_matrix, dims=([0], [2])).T

        return decoded

    def decode_empty(self, get_heat_map: bool = False) -> torch.tensor:
        """
        Decodes the unoccupied space in the environment.

        Args:
            get_heat_map (bool): Whether to return the decoded tensor as a heat map.

        Returns:
            torch.tensor: The decoded tensor.
        """
        # vs = [spf.invert(self.label_vectors["empty"]), self.memory]
        # decoded = spf.bind(vs, self.device)

        if get_heat_map:
            decoded = torch.tensordot(self.empty_memory, self.xy_axis_matrix, dims=([0], [2])).T

        return decoded


class LidarOccupancyGridMapper2D_RandomQuadrants:
    """
    A class that maps lidar data to a 2D occupancy grid using random quadrants.

    Attributes:
    - axis_limit (int): The limit of the x and y axis.
    - axis_resolution (int): The resolution of the x and y axis.
    - device (str): The device to use for computation.
    - length_scale (float): The length scale for the SSPs.
    - vsa_dimensions (int): The number of dimensions for the vectors.
    - label_vectors (dict): A dictionary containing the label vectors.
    - axis_bounds (torch.tensor): A tensor containing the axis bounds.
    - quadrant_vectors (list): A list containing the quadrant vectors.
    - quadrant_centers (torch.tensor): A tensor containing the quadrant centers.
    - pdist (torch.nn.PairwiseDistance): A pairwise distance function.
    - axis_basis_vectors (torch.tensor): A tensor containing the axis basis vectors.
    - xy_axis_matrix (torch.tensor): A tensor containing the xy axis matrix.
    - memory (torch.tensor): A tensor containing the memory.
    """

    def __init__(self, config: DictConfig, device: torch.device) -> None:
        """
        Initializes a Mapping object with default parameters.
        ...
        self.axis_resolution: int = config.axis_resolution
        self.device: torch.device = device
        self.length_scale: float = config.length_scale
        self.num_quads: int = config.num_quadrants
        self.vsa_dimensions: int = config.vsa_dims
        self.world_shape: np.array = config.world_shape
        ...
        self.quadrant_vectors = [
            [
                spf.make_good_unitary(self.vsa_dimensions, self.device, length_scale=self.length_scale) for _ in range(self.num_quads)
            ] for _ in range(self.num_quads)
        ]

        self.quadrant_centers_x = torch.linspace(self.world_shape[0], self.world_shape[1], self.num_quads + 2, device=self.device)
        self.quadrant_centers_y = torch.linspace(self.world_shape[2], self.world_shape[3], self.num_quads + 2, device=self.device)

        self.quadrant_centers_x = self.quadrant_centers_x[1:-1] # get rid of the values at the end
        self.quadrant_centers_y = self.quadrant_centers_y[1:-1] # get rid of the values at the end

        self.quadrant_centers = torch.stack(torch.meshgrid(self.quadrant_centers_x, self.quadrant_centers_y), dim=2)
        ...
        # -----------------------------------
        x_axis = torch.linspace(
            start=self.world_shape[0],
            end=self.world_shape[1],
            steps=int((self.world_shape[1] - self.world_shape[0]) / self.axis_resolution),
            device=self.device
        )
        x_axis = x_axis.reshape((-1, 1))

        y_axis = torch.linspace(
            start=self.world_shape[2], 
            end=self.world_shape[3],
            steps=int((self.world_shape[3] - self.world_shape[2]) / self.axis_resolution),
            device=self.device
        )
        y_axis = y_axis.reshape((-1, 1))

        self.axis_basis_vectors: torch.tensor = ssp_gen.generate(2)

        self.xy_axis_matrix = torch.zeros((x_axis.shape[0], y_axis.shape[0], self.vsa_dimensions), device=self.device)
        for i, x in enumerate(x_axis):
            for j, y in enumerate(y_axis):
                quadrant_distances = self.pdist(torch.tensor([x, y], device=self.device), self.quadrant_centers)
                closed_quad = torch.argwhere(quadrant_distances == torch.min(quadrant_distances))[0]
                vs = [
                    spf.power(self.axis_basis_vectors[0], x, self.length_scale),
                    spf.power(self.axis_basis_vectors[1], y, self.length_scale),
                    self.quadrant_vectors[closed_quad[0]][closed_quad[1]],
                ]
                self.xy_axis_matrix[i, j, :] = spf.bind(vs, self.device)
        ...
        # --------------
        self.occupied_memory = torch.zeros(self.vsa_dimensions, device=self.device)
        self.unoccupied_memory = torch.zeros(self.vsa_dimensions, device=self.device)
    ...
    def process_lidar(self, lidar_matrix: torch.tensor) -> None:
        """
        TODO Finish Documentation
        """
        ...
        if not isinstance(lidar_matrix, torch.Tensor):
            lidar_matrix = torch.from_numpy(lidar_matrix)
        
        lidar_matrix = lidar_matrix.to(self.device)

        # lidar_matrix -= self.axis_limit

        for i in range(lidar_matrix.shape[0]):
            lv = spe.encode_cartesian(
                axis_basis_vectors=self.axis_basis_vectors,
                axis_values=lidar_matrix[i, :2],
                device=self.device,
                length_scale=self.length_scale
            )

            quadrant_distances = self.pdist(lidar_matrix[i, :2], self.quadrant_centers)
            closed_quad = torch.argwhere(quadrant_distances == torch.min(quadrant_distances))[0]
            vs = [
                lv,
                self.quadrant_vectors[closed_quad[0]][closed_quad[1]],
            ...
            if lidar_matrix[i, 2] == 1:
                self.occupied_memory += spf.bind(vs, self.device)
            else:
                self.unoccupied_memory += spf.bind(vs, self.device)


    def decode_occupied(self) -> Tuple[torch.Tensor, torch.Tensor]:
        """
        Decodes the label vectors for the "occupied" class and returns the decoded tensor and a clone of the xy axis matrix.

        Returns:
            A tuple containing:
            - decoded (torch.Tensor): The decoded tensor for the "occupied" class.
            - xy_axis_matrix (torch.Tensor): A clone of the xy axis matrix.
        """
        vs = [spf.invert(self.quadrant_vectors[0][0]), self.occupied_memory]
        decoded = spf.bind(vs, self.device)

        return self.occupied_memory, self.xy_axis_matrix
    
    def decode_heatmap(self) -> np.ndarray:
        """
        Decodes the occupancy grid into a heatmap.

        The heatmap is a 2D numpy array where each cell represents the probability of occupancy
        for the corresponding cell in the occupancy grid. The values are normalized between 0 and 1.

        Returns:
            np.ndarray: A 2D numpy array representing the heatmap.
        """
        decoded_v, axis_matrix = self.decode_occupied()
        hm = torch.tensordot(decoded_v, axis_matrix, dims=([0], [2]))
        hm = hm.cpu().numpy()
