...
VALID_DATASETS: List[str] = ["toysim", "ablation", "intel", "pickle"]
...
def load_data(config: DictConfig, loggers: List[BaseLogger]):
    ...
    elif dataset_name == "pickle" and exp_type == "multi_agent":
        from vsa_ogm.data.sa.pickle_sa_dataset import PickelSingleAgentDataset
        dataset = PickelSingleAgentDataset(config, loggers)
    else:
        raise ValueError(f"Unknown dataset - {dataset_name} - must be in [{VALID_DATASETS}]")
